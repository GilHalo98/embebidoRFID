...
respuestaIoT INTERFAZ::registrarReporteAcceso(void) {
    /*
    ...
    */
    ...
    String uri = String("http://")
        ...
        + String(PORT_API)
        + String(VERSION_API)
        + String(ENDPOINTS::registrarReporteAcceso);
    ...
    if(registroHTTP.begin(cliente, uri)) {
        ...
        if(httpCode > 0) {
            ...
            // Si el codigo no es valido, la peticion retorna un
            // codigo http invalido.
            Serial.printf(
...
/*
void registrarReporteAccesoSOCKETS(
    ...
    // add evnet name
    array.add("registrar_acceso");

    // add payload (parameters) for the event
    JsonObject param1 = array.createNestedObject();
    param1["authorization"] = ACCESS_TOKEN;
    param1["resolucion"] = (uint8_t) ACCESO_GARANTIZADO;
    param1["tipoReporte"] = (uint32_t) tipoReporte;
    param1["descripcionReporte"] = descripcionReporte;
    param1["empleado"] = ID_EMPLEADO;
